...
#include "img_bins/ditto/diego.h"
...
{
	OAM_Mem[2] = 0x1400;
	LZ77UnCompVram(&mikachuTiles, &OBJData[0]);
}
...
void loadDittoGFX()
{
	...
	copyPal(&mikachuPal, &OBJ_PaletteMem[16]);
	copyPal(&diegoscreenPal, &BG_PaletteMem[0]);
	LZ77UnCompVram(&ditto1Tiles, &OBJData[0]);
	LZ77UnCompVram(&diegoscreenTiles, &Tiles[0]);

	LZ77UnCompVram(&diegoscreenMap, &MapData[0]);
